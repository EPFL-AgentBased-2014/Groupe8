...
to wait-if-necessary ;Attendre le nombre de ticks spécifiés
  if [pcolor] of patch-here = red or patch-here = patch -33 3 or patch-here = patch -6 3 or patch-here = patch 5 -3
  [
...
to move-if-you-can
  ...
  set goahead true
    if [pcolor] of patch-here = red or patch-here = patch -33 3 or patch-here = patch -6 3 or patch-here = patch 5 -3 and not waitingTimeOver [ set goahead false ]     ; Si j'ai pas fini mon waiting time -> wait
...
to adjustmyspeed                                                                  ;in order to not sto twice on a station (if speed <1) set the speed to 1 if I am on a station
  ifelse [pcolor] of patch-here = red or patch-here = patch -33 3 or patch-here = patch -6 3 or patch-here = patch 5 -3
  [                                           ;in the next step set it on speed again
    set myspeed 1
  ][
